...
frappe.query_reports["INV05 - Thẻ Kho"] = {
    "filters": [
        {
        ...
        },
        {
            ...
            on_change: function (query_report) {
                ...
                if (item) {
                    frappe.db.get_value("Item", item, ["item_name", "stock_uom"], (r) => {
                        frappe.query_report.set_filter_value("item_name", r.item_name);

                        frappe.db.get_value("UOM", r.stock_uom, "uom_name", (p) => {
                            frappe.query_report.set_filter_value("uom_name", p.uom_name);
                        })
                    });
                } else {
                    frappe.query_report.set_filter_value("item_name", "");
                    frappe.query_report.set_filter_value("uom_name", "");
                }
            },
        },
        {
